import type{ItemSlot}from"./content";import{worldIndexForZone}from"./worlds";
...
export function rollItem(zone:number,enemyIndex:number,victories:number,lootMultiplier=1):ItemInstance{
 const seed=zone*10007+(victories+1)*97+enemyIndex*13,slots=["weapon","armor","charm"] as const,slot:ItemSlot=slots[Math.floor(hash(seed+1)*slots.length)]??"weapon",rarity=rollRarity(zone,lootMultiplier,seed+2),rank=rarityRank(rarity),itemLevel=Math.max(1,zone+Math.floor(victories/12)),roll=.86+hash(seed+3)*.3,base=Math.round((3+itemLevel*1.65)*(1+rank*.42)*roll);
 ...
 const biomeNames=["Himmel","Glut","Frost","Prisma","Sonne","Leere"] as const,biome=biomeNames[worldIndexForZone(zone)]??"Himmel",prefix=prefixes[Math.floor(hash(seed+4)*prefixes.length)]??"Runen",slotBases=bases[slot],baseName=slotBases[Math.floor(hash(seed+5)*slotBases.length)]??slotBases[0]??"Relikt",name=`${rank>=2?prefix+" ":""}${biome}-${baseName}`;
 ...
 const sets:ItemSet[]=["sky","ember","frost","prism","sun","void"],set:ItemSet|undefined=rank>=2?(sets[worldIndexForZone(zone)]??"sky"):undefined;return{uid:`${seed}-${Math.floor(hash(seed+6)*99999)}`,set,baseId:`${biome.toLowerCase()}-${slot}`,name,slot,rarity,itemLevel,power:slot==="weapon"?base+affAtk:Math.round(base*.32)+affAtk,hp:slot==="armor"?base*4+affHp:affHp||undefined,crit:slot==="charm"?2+rank*2+affCrit:affCrit||undefined,affixes}
}
