import{worldForZone}from"./worlds";
import{encounterForStage}from"./encounters";
...
export function enemiesForZone(zone:number):EnemyDefinition[]{const w=(Math.max(1,zone)-1)%6,world=worldForZone(zone),scale=1+(Math.max(1,zone)-1)*.22;return[32,48,70,96,165].map((hp,i)=>{const kind=encounterForStage(i);return{id:world.family+"-"+i,name:names[w][i],maxHp:Math.round(hp*scale),attack:Math.max(2,Math.round([2,3,4,5,8][i]*(1+(zone-1)*.12))),xp:Math.round([10,14,20,28,55][i]*scale),gold:Math.round([6,9,13,18,36][i]*scale),essence:i===4?3:1,tier:kind==="boss"?"boss":kind==="tough"?"tough":"normal",family:world.family}})}
