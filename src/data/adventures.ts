...
export type AdventureRoom="battle"|"elite"|"treasure"|"boss";export type AdventureEnemy={name:string;icon:string;hp:number;attack:number;rewardGold:number;rewardEssence:number;boss:boolean;room:AdventureRoom;intent:string;modifier:string};
...
export function adventureEnemy(mode:AdventureMode,wave:number,zone:number,floor:number):AdventureEnemy{
 ...
 const modifier=room==="treasure"?"Vergoldet: mehr Beute":room==="elite"?"Elite: +35% Leben":boss?(mode==="tower"?"Turmgesetz: kein Rückzug ohne Verlust":"Bosskammer: erhöhter Druck"):wave===0?"Vorhut: stabiler Einstieg":"Kampfkammer: Combo aufbauen";
 return{name:names[wave%names.length]??"Wächter",icon:boss?"♛":room==="treasure"?"▣":room==="elite"?"✦":mode==="tower"?"♜":"◆",hp:Math.round((boss?150:65)*scale*elite*treasure),attack:Math.round((boss?9:4)*scale*(room==="elite"?1.25:1)),rewardGold:Math.round((boss?75:room==="treasure"?55:24)*scale),rewardEssence:boss?2:room==="elite"?1:0,boss,room,intent:boss?"Phasenangriff":room==="elite"?"Schwerer Schlag":room==="treasure"?"Schatz bewachen":"Angriff",modifier};
}
