#!/usr/bin/env python3
"""V2.05 balance snapshot — combat + daily/quest/chest overlays (offline, mirrors GameConfig)."""
import math
...
def meta_bonus_for_minutes(minutes: float) -> int:
    """Rough daily + quest + chest cadence (not dominant vs combat)."""
    daily = DAILY_GOLD_AVG if minutes >= 5 else 0
    quests = int(minutes / 15) * QUEST_GOLD_AVG
    chests = int(minutes / 30) * CHEST_GOLD_AVG
    return daily + quests + chests
...
def simulate(minutes: float, taps_per_sec: float = 2.5):
    ...
    while elapsed < sec:
        ...
        if monster_hp_cur <= 0:
            reward = monster_reward(monster_level)
            gold += reward
            combat_gold += reward
            kills += 1
            monster_level += 1
            ...
            if gold >= cost:
                ...
                upgrades += 1
    meta = meta_bonus_for_minutes(minutes)
    afk = int(round(minutes * AFK_GOLD_PER_MIN)) if minutes * 60 >= AFK_MIN_SEC else 0
    net_combat = gold
    total = net_combat + meta + afk
    return {
        "minutes": minutes,
        "kills": kills,
        "upgrades": upgrades,
        "combat_gold_gross": combat_gold,
        "combat_gold_net": net_combat,
        "meta_gold": meta,
        "afk_gold": afk,
        "total_gold": total,
        "tap_level": tap_level,
        ...
        "combat_share_pct": round(net_combat / max(total, 1) * 100, 1),
    }
